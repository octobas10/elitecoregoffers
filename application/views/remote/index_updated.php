...
function add_offer_to_page(){
	...
	<?php 
	/**
	 * @Since : 12/01/2017 (13:58)
	 * @Description : pass the parameter in action of the form.
	 *                this parameter contain the value which is previously posted data.
	 */ 
	$s_passed_parameter = '';
			...
			foreach ($list_offer as $offer) {
				...
				<?php } else { 
							...
							htmlform += "<div><select name='offer_<?php echo $offer_id;?>[<?php echo $field_name;?>]' class='coreg_fields <?php if(isset($offer_field->required) && $offer_field->required==1){echo "require";}?> <?php echo $field_name;  ?>'><option value=''>--Select <?php echo str_replace('_',' ',ucfirst($offer_field->label))?>--<\/option>";
								...
								htmlform += "<option value='<?php echo $offer_field->value->keys[$rcount];?>' <?php echo (isset($t_posted_array[$field_name]) && ($t_posted_array[$field_name] == $select)  ? 'selected' : '' ); ?> <?php if(isset($offer_field->value->checked) && $offer_field->value->checked==$key){echo"selected";}?>><?php echo $select;?><\/option>";
								<?php 
								$rcount++;
								}?>
							htmlform += "<\/select></div>";			

		...
		return this.each(function () {
			...
			var appendNewField = function (type, values, options, required) {
					...
					case 'select':
						...
						break;
					case 'hidden':
						appendHiddenInput(values, required);
						break;	
					}
				};
			
			//Hidden single line input type="hidden"
			var appendHiddenInput = function (values, required) {
					field += '<div class="frm-fld"><label>' + opts.messages.label + '</label>';
					field += '<input class="fld-title" id="title-' + last_id + '" name="title" type="text" value="' + values + '" />';
					field += '</div><div class="frm-fld"><label>' + opts.messages.hidden_field_value + '</label>';
					field += '<input type="text" name="hidden_value" value="" /></div>';
					help = '';
					appendFieldLi(opts.messages.hidden, field, required, help);
				};
			...
			var save = function () {
				/*var formdata = $(ul_obj).serializeFormList({prepend: opts.serialize_prefix});
				var data = $("#offer_image_box").html();
				data += $("#offer_text_box").html();
				$("#offer_form").html(formdata);				
				$("#offer_template").html(data);
				$(".close").click();*/
				
				var data = "<ul style=\"list-style-type: none; margin: 0;width: 600px;border: 1px solid #cccccc;min-height: 400px;height: 100%;background: white;float: left;\">";
				$( "#sortable li" ).each(function( index ) {
					var data_id =$(this).attr('id');
					data += "<li>";
					if(data_id == "offer_image_container")
					{
						data += $("#offer_image_box").html();
					}
					else if(data_id == "offer_text_container")
					{
						data += $("#offer_text_box").html();
					}
					else
				...
				for (att = 0; att < opts.attributes.length; att++) {
					...
					if (opts.attributes[att] === 'class') {
						...
						case 'hidden':
							...
							serialStr += opts.prepend + '[' + li_count + '][values]=' + encodeURIComponent($('#' + $(this).attr('id') + ' input[name=hidden_value]').val());
							break;
						case 'input_text':
							serialStr += opts.prepend + '[' + li_count + '][title]=' + encodeURIComponent($('#' + $(this).attr('id') + ' input[type=text]').val());
							break;
